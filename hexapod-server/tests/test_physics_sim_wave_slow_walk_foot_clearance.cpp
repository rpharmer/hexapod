...
std::string waveSlowWalkFootClearanceLimitsJson(const double min_foot_tip_world_z_m,
...
int main(int argc, char** argv) {
#if !defined(__linux__)
    std::cout << "skip test_physics_sim_wave_slow_walk_foot_clearance (Linux-only)\n";
    return 0;
#else
    ...
    if (sim_exe == nullptr || sim_exe[0] == '\0') {
        std::cout << "skip test_physics_sim_wave_slow_walk_foot_clearance (pass sim path or HEXAPOD_PHYSICS_SIM_EXE)\n";
        return 0;
    }

    constexpr const char* kSuite = "physics_sim_wave_slow_walk_foot_clearance";
    constexpr const char* kCase = "wave_slow_walk_foot_clearance";
    ...
    const auto limitsJson = [&]() {
        return waveSlowWalkFootClearanceLimitsJson(kMinFootTipWorldZ, kRequireSawRawContactLoss, kMaxBodyUndershootM);
    };
    ...
    if (!expect(runtime.init(), "runtime init should succeed against the live physics sim")) {
        if (emit_metrics_json) {
            physics_sim_metrics::emitLine("physics_sim_wave_slow_walk_foot_clearance", "wave_slow_walk_foot_clearance", false,
    ...
    // Scenario 05, t=30-39 s: WAVE 0.05 m/s — the lowest swing height of all slow phases
    // (floor ≈ 20.5 mm) making it the worst-case for foot scraping.  WAVE duty factor 0.82
    // means each leg is in swing for only 18% of the cycle, further compressing the arc.
    const ScenarioMotionIntent stand_motion{true, RobotMode::STAND, GaitType::TRIPOD, 0.14, 0.0, 0.0, 0.0};
    const ScenarioMotionIntent walk_motion{true, RobotMode::WALK, GaitType::WAVE, 0.14, 0.05, 0.0, 0.0};
    ...
    if (!expect(bridge_ptr->last_state().has_value(), "bridge should produce an initial state before WAVE walk")) {
        if (emit_metrics_json) {
            physics_sim_metrics::emitLine("physics_sim_wave_slow_walk_foot_clearance", "wave_slow_walk_foot_clearance", false,
    ...
    for (int i = 0; i < kWalkObserveSteps; ++i) {
        ...
        if (!expect(bridge_ptr->last_state().has_value(), "bridge should keep producing live state during WAVE walk")) {
            ::kill(pid, SIGTERM);
            ::waitpid(pid, nullptr, 0);
            if (emit_metrics_json) {
    ...
    ok = expect(min_foot_tip_world_z_m >= kMinFootTipWorldZ,
                "slow WAVE walk should keep the reported server foot tip at or above the ground plane") &&
         ok;
    ok = expect(!kRequireSawRawContactLoss || saw_any_raw_contact_loss,
                "slow WAVE walk should see at least one swing-leg contact loss confirming feet lift") &&
         ok;
    ok = expect(max_body_undershoot_m < kMaxBodyUndershootM,
                "body height should track within 10 mm of commanded 0.14 m during slow WAVE walk") &&
         ok;
    if (emit_metrics_json) {
        std::ostringstream metrics;
