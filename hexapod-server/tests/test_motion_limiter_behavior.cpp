...
bool scenario_body_leads_on_start_and_legs_lead_on_stop() {
    ...
    MotionIntent idle{};
    idle.requested_mode = RobotMode::SAFE_IDLE;
    (void)pipeline.runStep(est, idle, safety, true, 1);

    MotionIntent walk = walkingIntent();
    std::this_thread::sleep_for(std::chrono::milliseconds(20));
    const PipelineStepResult started = pipeline.runStep(est, walk, safety, true, 2);

    MotionIntent stop = walk;
    stop.requested_mode = RobotMode::SAFE_IDLE;
    std::this_thread::sleep_for(std::chrono::milliseconds(20));
    const PipelineStepResult stopped = pipeline.runStep(est, stop, safety, true, 3);
