...
public:
    ...
    void publishControlStep(const ControlStepTelemetry& telemetry) override {
        ...
        payload << "{\"type\":\"joints\",\"schema_version\":1,"
                ...
                << "\"stages\":{"
                << "\"navigation\":" << (telemetry.status.autonomy.stages.navigation ? "true" : "false") << ","
                << "\"localization\":" << (telemetry.status.autonomy.stages.localization ? "true" : "false") << ","
                << "\"world_model\":" << (telemetry.status.autonomy.stages.world_model ? "true" : "false") << ","
                << "\"traversability\":" << (telemetry.status.autonomy.stages.traversability ? "true" : "false") << ","
                << "\"global_planner\":" << (telemetry.status.autonomy.stages.global_planner ? "true" : "false") << ","
                << "\"local_planner\":" << (telemetry.status.autonomy.stages.local_planner ? "true" : "false") << ","
                << "\"progress_monitor\":" << (telemetry.status.autonomy.stages.progress_monitor ? "true" : "false") << ","
                << "\"recovery_manager\":" << (telemetry.status.autonomy.stages.recovery_manager ? "true" : "false") << ","
                << "\"motion_arbiter\":" << (telemetry.status.autonomy.stages.motion_arbiter ? "true" : "false") << ","
                << "\"locomotion_interface\":"
                << (telemetry.status.autonomy.stages.locomotion_interface ? "true" : "false")
                << "}"
                << "}}},"
                << "\"dynamic_gait\":{"
