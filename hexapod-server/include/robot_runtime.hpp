...
private:
    ...
    std::atomic<uint64_t> raw_sample_id_{0};
    std::atomic<uint64_t> estimator_sample_id_{0};
    std::atomic<uint64_t> intent_sample_id_{0};
    TimePointUs last_control_step_us_{};

    DoubleBuffer<StreamSample<RawHardwareState>> raw_state_;
    DoubleBuffer<StreamSample<EstimatedState>> estimated_state_;
    DoubleBuffer<StreamSample<MotionIntent>> motion_intent_;
