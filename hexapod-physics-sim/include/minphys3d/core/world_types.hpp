...
struct ContactKey {
    ...
    bool operator==(const ContactKey& other) const {
        return loBody == other.loBody
            && hiBody == other.hiBody
            && manifoldType == other.manifoldType
            && canonicalFeatureId == other.canonicalFeatureId;
    }
};
...
struct ContactKeyHash {
    std::size_t operator()(const ContactKey& key) const noexcept {
        ...
        seed ^= static_cast<std::size_t>(key.canonicalFeatureId) + 0x9e3779b9u + (seed << 6u) + (seed >> 2u);
        seed ^= static_cast<std::size_t>(key.canonicalFeatureId >> 32u) + 0x9e3779b9u + (seed << 6u) + (seed >> 2u);
        return seed;
    }
};
