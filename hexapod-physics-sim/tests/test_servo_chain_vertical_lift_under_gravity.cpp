...
struct VerticalLiftMetrics {
    Real peak_payload_height = 0.0;
    Real initial_payload_height = 0.0;
    Real final_angle = 0.0;
    int first_step_reaching_target = -1;
    bool finite = true;
};

VerticalLiftMetrics runVerticalLift(Real max_servo_torque) {
    World world({0.0, -9.81, 0.0});
    const std::uint32_t base_id = world.CreateBody(MakeStaticBase());

    constexpr Real kArmLength = 1.0;
    const std::uint32_t arm_id = world.CreateBody(MakeArmLink({0.5 * kArmLength, 0.0, 0.0}, kArmLength, 0.7));

    Body payload;
    payload.shape = ShapeType::Sphere;
    payload.radius = 0.07;
    payload.mass = 8.0;
    payload.position = {kArmLength, 0.0, 0.0};
    payload.restitution = 0.0;
    payload.linearDamping = 0.02;
    payload.angularDamping = 0.02;
    const std::uint32_t payload_id = world.CreateBody(payload);
    world.CreateFixedJoint(arm_id, payload_id, {kArmLength, 0.0, 0.0});
    ...
    for (int step = 0; step < 720; ++step) {
        ...
        if (metrics.first_step_reaching_target < 0 && world.GetServoJointAngle(servo_id) >= 1.0) {
...
int runCase() {
    ...
    const Real high_lift = high_torque.peak_payload_height - high_torque.initial_payload_height;
    if (high_lift < 0.20) {
    ...
    if (high_torque.final_angle < 0.50) {
