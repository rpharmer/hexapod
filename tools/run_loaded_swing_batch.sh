#!/usr/bin/env bash
# A/B batch for the contact-aware liftoff screen. One process per run, separate
...
for screen in reverse_walk turn_in_place sequential; do
  for arm in baseline candidate; do
    for i in $(seq 1 "$runs"); do
      ...
      "$root/tools/run_loaded_swing_screen.sh" "$out" "$screen" "$arm" \
...
for arm in baseline candidate; do
  ...
  [ -f "$stem.stdout" ] && continue
  "$root/tools/run_loaded_swing_screen.sh" "$out" aggressive_governor "$arm" \
    > "$stem.stdout" 2> "$stem.stderr"
