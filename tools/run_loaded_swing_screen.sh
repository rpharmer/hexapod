#!/usr/bin/env bash
# Contact-aware liftoff screen. Clean HEXAPOD_* subshell per leftovers §8, then
# one walk-distance process. Candidate is opt-in via HEXAPOD_LOADED_SWING_HOLD.
#
# Usage: tools/run_loaded_swing_screen.sh <out-dir> <screen> <baseline|candidate> [error_rad]
...
(
  ...
  if [ "$arm" = candidate ]; then
    export HEXAPOD_LOADED_SWING_HOLD=1
    if [ -n "$error_rad" ]; then
      export HEXAPOD_LOADED_SWING_HOLD_ERROR_RAD="$error_rad"
    fi
  fi
